	...
	function Monster(monsterData){
		...
		this.init = function(monsterData){
			...
			if(self.fullyDynamicStats){
				...
				self.hp( Math.round( Utils.doRand( Math.ceil(avgMonsterHp * 0.7), Math.ceil(avgMonsterHp * 1.3) ) ) );
				self.minDmg( Math.ceil(avgMonsterDmg * 0.7) );
				self.maxDmg( Math.ceil(avgMonsterDmg * 1.3) );
